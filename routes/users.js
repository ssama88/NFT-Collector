...
router.get("/pics", async (req, res) => {
  try {
    ...
    NFTs.map((nft) => {
      nft.description = `Sold for $${nft?.price}  million USD at the time of the transaction`;
      return nft;
    });
    res.send(NFTs);
  } catch (error) {
    console.log("error", error);
    res.send(JSON.stringify("Unable to retrieve images"));
  }
});
...
router.patch("/:username", async (req, res) => {
  ...
  if (!findUser) res.send("error 404 page not found");
  if (url)
    findUser.profilePic = url; //if the user wants to change their profile image
  else if (newpassword)
